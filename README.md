...
- **Phase 0** 설계 확정 ✅
- **Phase 1** 기반 구축 — 프로젝트 골격 / 시트 스키마·시드 / 로그인·권한 뼈대 ✅
- **Phase 2** 체크리스트 작성 페이지 ✅
- **Phase 3** 결과 대시보드 (필터·경고·매장 상태·항목 취약점·상세 보고서) ✅
- **Phase 4** 매장별 종합 관리 (일/주/월/연 추이·누적 통계·재정산/재점검 이력·CSV/PDF) ✅
- **Phase 5** 마감 — 실제 Apps Script 배포, OAuth 연결, 사용자 이메일 등록, 운영 전환 (남음)
...
실제 코드는 Google Apps Script 에 배포해야 동작한다. 배포 전에 **화면과 흐름만** 보려면:

```bash
node preview/build.js       # preview/index.html 생성 (목업 데이터 주입)
```

생성된 `preview/index.html` 을 브라우저로 그냥 열면 된다(더블클릭 가능).
백엔드 없이 목업 데이터로 로그인(그룹장/총괄/대표) → 3개 탭 이동까지 확인할 수 있다.
저장·Google 로그인은 미리보기에서 동작하지 않는다.
...
- 10개 항목 **모두 중요항목**. 항목 중 X 표시가 2개 이상 → 재점검 필요
